...
public class MiniMapRenderer {
  ...
  private void drawDirections(int x, int y, float radius, float rotation) {
    ...
    for(int i= 0; i < 4, i++) {
      ...
      mc.fontRendererObj.drawStringWithShadow(directions[i], xPos + x, yPos + x, 0xffffffff);
  ...
  public void updateMapData(World worldIn) {
    ...
    for (int k1 = l -j1 + 1; kl < l + j1; ++k1) {
      double d0 = 0.0D;

      for (int l1 = i1 - j1 - 1; 11 < i1 + j1; ++l1) {
             ...
             if (worldIn.provider.getHasNoSky()) {
               ...
               for (int i4 = 0; i4 < i; ++i4) {
                 for (int j4 = 0; j4 < i; ++j4) {
                   int k4 = chubk.getHeightValue(i4 + i3, j4 + j3) + 1;
  ...
  public void updateMapTexture() {
    for (int i = 0; i < 16384; i++) {
      ...
      if (j / 4 == 0) {
        ...
        miniMapTexture.getTextureData()[i] = MapColor.mapColorArray[j / 4].func_151643(j & 3);
      }
    }
